...
#        print 'self.image shape=%s , value=%s' % (str(self.image.shape),str(self.image))     #<<<140903>>>
    ...
    def setCentroid(self):
        print ('--------- inside setCentroid  -------- ')
        y,x = numpy.indices(self.image.shape).astype(numpy.float32)
#        print 'x= %s, y= %s' % (str(x), str(y))     #<<<140903>>>
        self.x0 = (x*self.image).sum()
        self.y0 = (y*self.image).sum()
        print ('self.x0= %s, self.y0= %s' % (str(self.x0), str(self.y0)))     #<<<140903>>>
