...
class Sersic(SBModel,SBProfiles.Sersic):
    ...
    def Mag(self,zp):
#        print 'self.amp=', self.amp         # <<<140825>>>
...
class PointSource(PM,GM):
    def __init__(self,name,model,pars):
        print ('------- inside PointSource -> __init__ ------')     #<<<140903>>>
        if 'amp' not in pars.keys():
            pars['amp'] = 1.
        print ('pars[x]=%s, pars[y]=%s, pars[amp]=%s' % (str(pars['x']),str(pars['y']),str(pars['amp'])))     #<<<140903>>>
        self.keys = pars.keys()
        self.keys.sort()
        print ('self.keys=' + str(self.keys))     #<<<140903>>>
        ...
        if type(model)==type([]):
            ...
            print ('use PM and type(model)=%s, shape(model)=%s' % (str(type(model)),str(model.shape)))     #<<<140903>>>
